...
def load_image_batch(files):
    ...
    """
    return np.array([remove_alpha_channel(imread(str(fn))).astype(np.float32) for fn in files])
...
def calculate_activation_statistics_from_files(files, sess, batch_size=50, verbose=False):
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the incption model.
    """
    files = random.sample(files, MAX_FILES_NUM)
...
def check_or_download_inception(inception_path):
    ...
    if inception_path is None:
        inception_path = 'C:/Users/Srijay/Desktop/Projects/pix2pix-tensorflow/tmp'
    inception_path = pathlib.Path(inception_path)
    model_file = inception_path / 'classify_image_graph_def.pb'
    print("Checking inception file ",model_file)
