...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    filter_shape = [11, 11, a.ngf, a.ngf]
    with tf.variable_scope("encoder_1_2"):
        filter = tf.get_variable('edge_detector', filter_shape, initializer=tf.random_normal_initializer(stddev=0.02))
        strides = [1, 1, 1, 1]
        output = tf.nn.conv2d(output, filter, strides=strides, padding='VALID')
        output = lrelu(output, 0.2)
        layers.append(output)

    filter_shape = [11, 11, a.ngf, a.ngf]
    with tf.variable_scope("encoder_1_3"):
        filter = tf.get_variable('edge_detector', filter_shape, initializer=tf.random_normal_initializer(stddev=0.02))
        strides = [1, 1, 1, 1]
        output = tf.nn.conv2d(output, filter, strides=strides, padding='VALID')
        output = lrelu(output, 0.2)
    ...
    filter_shape = [11, 11, a.ngf, a.ngf]
    with tf.variable_scope("encoder_1_4"):
        filter = tf.get_variable('edge_detector', filter_shape, initializer=tf.random_normal_initializer(stddev=0.02))
        strides = [1, 1, 1, 1]
        output = tf.nn.conv2d(output, filter, strides=strides, padding='VALID')
        output = lrelu(output, 0.2)
    ...
    with tf.variable_scope("decoder_1"):
        input = tf.concat([layers[-1], layers[4]], axis=3)
        rectified = tf.nn.relu(input)
