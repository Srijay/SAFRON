...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        ...
        if a.lab_colorization:
            ...
            sess = tf.Session()
            print("Here ", sess.run(tf.shape(width)))
...
strides_rows = 256
strides_cols = 256
num_patches = int(a.scale_size/ksize_rows)
...
def extract_patches_inverse(x, y):
    _x = tf.zeros_like(x)
    _y = extract_patches(_x,ksizes, strides, rates)
...
def create_model(inputs, targets):
    ...
    with tf.variable_scope("extract_patches"):
        inputs_patches = extract_patches(inputs, ksizes, strides, rates)
        ...
        with tf.variable_scope("generator", reuse=tf.AUTO_REUSE):
            ...
            for i in range(0, num_patches):
                for j in range(0, num_patches):
                    ...
                    patch_size_len = int(patch.get_shape()[0])
                    # reshape
            ...
            outputs = extract_patches_inverse(inputs, output_patches)
...
def save_images(fetches, step=None):
    ...
    for i, in_path in enumerate(fetches["paths"]):
        ...
        for kind in ["inputs", "outputs_test", "targets"]:
...
def main():
    ...
    with tf.name_scope("encode_images"):
        display_fetches = {
            ...
            "outputs_test": tf.map_fn(tf.image.encode_png, converted_outputs, dtype=tf.string, name="output_pngs"),
        }
